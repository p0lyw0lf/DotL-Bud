...
class Permissions(Shell):
    ...
    def can_run_command(self, user, server, command):
        ...
        if admin_role_id in {str(role.id) for role in user.roles}:
            return True
