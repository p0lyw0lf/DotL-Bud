...
class DiceCommands(VariableCommands):
    def __init__(self, client, *args, **kwargs):
        ...
        for dice_size in DICE_LIST:
            self.commands.update({
                "d" + dice_size:
                    {"args": ["user", "str"],
                        "func": self._roll_dice_generator(dice_size)}
            })
    ...
    async def roll_dice(self, user, dice_size=None, num_dice='1'):
        ...
        out = dict()
        out["Dice size"] = (str(num_dice) + " x " if num_dice >
                            1 else "") + "d" + str(dice_size)
        result = 0
        for x in range(num_dice):
            result += randint(1, dice_size)
        # message = self.db[(user.id, "dice", str(dice_size), str(result))]
        # + (" " if message else "") + message
        out["Result"] = "You rolled a " + \
            ("total of " if num_dice > 1 else "") + str(result)
        return out
