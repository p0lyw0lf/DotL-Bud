...
class MemeCommands(VariableCommands):
    ...
    async def register_meme(self, server, meme=None, do_update_db=True):
        ...
        if do_update_db:
            all_memes = ent2list(
                self.db["memes_of", str(server.id)], exclude={meme})
    ...
    async def add_to_meme(self, server, meme=None, link=None):
        ...
        all_links = ent2list(
            self.db["memes_in", str(server.id), meme], exclude={link})
    ...
    async def show_all_memes(self, server):
        ...
        if all_memes:
            meme_dict = dict()
            for meme in all_memes:
                meme_dict[meme] = "{} images".format(
                    len(ent2list(self.db["memes_in", str(server.id), meme])))
            return meme_dict
    ...
    async def delete_from_meme(self, server, meme=None, index=None):
        ...
        all_links = [all_links[x]
                     for x in range(len(all_links)) if x+1 != index]
